...
export function Picker({ value, onChange, options, placeholder = "Select…", width = 220, size = "md", block = false, direction = "down", className = "", searchable: searchableProp }: { value: string; onChange: (v: string) => void; options: PickerOption[]; placeholder?: string; width?: number; size?: "sm" | "md"; block?: boolean; direction?: "down" | "up"; className?: string; searchable?: boolean }) {
  ...
  return (
    <Popover align="left" width={width} block={block} direction={direction} onToggle={(o) => { if (!o) setQ(""); }} trigger={(open) => (
      <span className={`inline-flex items-center justify-between gap-2 ${h} px-2.5 rounded-ctl border bg-surface transition-colors ${block ? "w-full" : ""} ${open ? "border-steel ring-2 ring-steel/30" : "border-ctl hover:border-dim"} ${className}`}>
      ...
      {searchable && (
        <div className="p-1.5 border-b border-rule2 sticky top-0 bg-surface" onClick={(e) => e.stopPropagation()}>
          <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search…" className="w-full box-border h-ctl-sm px-2 rounded-ctl-sm border border-ctl bg-surface font-sans text-[12.5px] text-ink outline-none focus:border-steel placeholder:text-faint" />
        </div>
      )}
  ...
  return (
    <div className={`flex-none inline-flex h-ctl rounded-ctl overflow-hidden border ${disabled ? "border-ctl" : "border-steel"}`}>
      <button type="button" disabled={disabled} onClick={onSubmit}
        className={`inline-flex items-center gap-1.5 px-3.5 text-[13px] font-semibold whitespace-nowrap transition-colors ${disabled ? "bg-paper text-faint cursor-not-allowed" : "bg-steel text-white hover:brightness-110"}`}>
        {icon}{label}
      </button>
      <Popover align="right" width={menuWidth} direction="up" trigger={(open) => (
        <span className={`inline-flex items-center h-full px-1.5 border-l cursor-pointer transition-[filter,background-color] ${disabled ? "bg-paper text-muted border-ctl hover:text-ink" : "bg-steel text-white border-l-white/25 hover:brightness-110"} ${open ? "brightness-110" : ""}`}>
